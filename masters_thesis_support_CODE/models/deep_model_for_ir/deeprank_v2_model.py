...
from models.deep_model_for_ir.custom_layers import SimilarityMatrix, MaskedConv2D, TermGatingDRMM_FFN
...
class DeepRankV2(ModelAPI):
    ...
    def _build_model(self):
        ...
        if USE_BIDIRECTIONAL:
            gru = Bidirectional(GRU(GRU_REPRESENTATION_DIM, kernel_regularizer=REGULARIZATION, activation=ACTIVATION_FUNCTION, name="aggregation_snippet_by_q_term"), merge_mode="concat")
        else:
    ...
    @staticmethod
    def load(path = '/backup/saved_models/deep_rankV2'):
        
        with open(os.path.join(path, "meta_data.p"),"rb") as file:
            meta_data = pickle.load(file)
        
        deep_rank_model = DeepRankV2(meta_data["vocab_size"],
                                     meta_data["emb_size"],
                                     meta_data["tk_mode"],
                                     path)
        
        #load weights
        deep_rank_model.document_score_model.load_weights('/backup/saved_models/deep_rankV2/deep_rank_weights.h5')
