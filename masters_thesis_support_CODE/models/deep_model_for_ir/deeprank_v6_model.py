...
from models.deep_model_for_ir.custom_layers import SimilarityMatrix, MaskedConv2D, TermGatingDRMM_FFN, MaskedSelfAttention
...
class DeepRankV6(ModelAPI):
    ...
    def snippet_interaction(self, tokenized_query, tokenized_doc, snippet_length=QUERY_CENTRIC_CONTEX):
    ...
    @staticmethod
    def load(path = '/backup/saved_models/deep_rankV6'):
        
        with open(os.path.join(path, "meta_data.p"),"rb") as file:
            meta_data = pickle.load(file)
        
        deep_rank_model = DeepRankV6(meta_data["vocab_size"],
                                     meta_data["emb_size"],
                                     meta_data["tk_mode"],
                                     path)
        
        #load weights
        deep_rank_model.document_score_model.load_weights('/backup/saved_models/deep_rankV6/deep_rank_weights.h5')
